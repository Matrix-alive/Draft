...
export default function Sidebar({ open, onClose }) {
  ...
  return <aside className={`sidebar ${open ? 'sidebar-open' : ''}`}>
    ...
    <div className="sidebar-footer"><NavLink to="/notifications" className="side-link"><Icon name="Bell" size={18} /><span>Notifications</span><span className="notification-dot" /></NavLink><div className="sidebar-help"><span>Need a hand?</span><ArrowUpRight size={16} /><small>Visit the resource centre</small></div><button className="side-link logout-button" onClick={handleSignOut}><LogOut size={18} /><span>Sign out</span></button></div>
  </aside>
}
